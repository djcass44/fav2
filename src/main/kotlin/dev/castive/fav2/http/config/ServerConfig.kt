...
package dev.castive.fav2.http.config

import dev.castive.fav2.util.EnvUtil
import dev.castive.fav2.util.asEnv
...
class ServerConfig(private val port: Int) {
	...
	fun getServer(): Server {
		val server = Server()
		val secure = EnvUtil.FAV_SSL_ENABLED.asEnv().toBoolean()
		val h2 = EnvUtil.FAV_HTTP2.asEnv("true").toBoolean()
	...
	private fun getSslContextFactory(h2: Boolean = true): SslContextFactory = SslContextFactory.Server().apply {
		keyStorePath = EnvUtil.FAV_SSL_KEYSTORE.asEnv()
		if(h2) {
			cipherComparator = HTTP2Cipher.COMPARATOR
			provider = "Conscrypt"
		}
		setKeyStorePassword(EnvUtil.FAV_SSL_PASSWORD.asEnv())
	}
}
